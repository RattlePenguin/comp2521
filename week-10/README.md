...
A max heap ensures that values in a level / depth (parent) will be greater than or equal to the level below (children).
A min heap keeps smaller values on top.
...
### Operations
The most important operation is heapify.
...
If values are in the wrong position (e.g. greater below in a max heap) then a swap is made between parent and child.
...
Then heapify maintains the heap property in $O(logn)$ time.
...
This requires some mathematical analysis, but the idea is the height factor in the swaps needed for each heapify operation converges to a constant value, resulting in $O(n)$.
